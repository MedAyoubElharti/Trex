...
class Plotter:
    ...
    def do_plot(self):
        ...
        if self.CMP:
            for traffic in self.CONST_TRAFFIC:
                ...
                os.system(
                    "/export/tmp/Downloads/hdr-plot-master/bin/hdr-plot --output out/CMP_{}.png --title '{}' {} ".format(
    ...
    def plot_multiple(self, hdrh_files, jitters):
        for i in range(len(hdrh_files)):
            file = hdrh_files[i]
            jitter = jitters[i]
            cmd = "/export/tmp/Downloads/hdr-plot-master/bin/hdr-plot --output {}.png --title '{}' {} ".format(
